...
sudo apt-get install -y build-essential libtool autopoint autoconf bison gtk-doc-tools libboost-all-dev gcc-multilib libedit-dev libclang-dev libcairo2-dev lld gcc-multilib g++-multilib &&
...
sudo apt-get install -y xinetd tftp &&
...
#Needed for Sparta
sudo apt install -y libyaml-cpp-dev rapidjson-dev libhdf5-serial-dev libwxgtk3.2-dev &&
